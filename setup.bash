...
aks () {
    ...
    if [ $? -eq 0 ]; then
        clear
        CHOICES=($VALUES)
        CMD="scripts/deploy-gpu-aks.bash --subscription ${CHOICES[0]} -g ${CHOICES[1]} --location ${CHOICES[2]} --template-file arm/deploy-gpu-aks.json --parameters-file arm/deploy-gpu-aks.parameters.json --create-aad-sp"
        echo "Issuing command => $CMD"
        echo ''
        sleep 3
        /bin/bash -c $CMD
